 ...
 * 分页分板块查询文章
 */
@WebServlet(name = "TopicPages", value = "/TopicPages")
public class TopicPages extends HttpServlet {
    @Override
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (categoryId == null || "".equals(categoryId)) {
            // 调用业务逻辑层方法
            list = ts.getTopicByPage(currentPage, pageSize);
        } else {
            list = ts.getTopicByPage(currentPage, pageSize, categoryId);
        }
